...
2. Click **New Project**
3. Name it (e.g. `going-green-advocacy-2`) and set a database password (use a dedicated project unless you intentionally share admin data with Advocacy 1)
...
Inserts from the main site use `state2026`; this city council site uses `city2026`. The `admin-sends` Edge Function requires JSON `{ "password": "...", "siteTag": "state2026" }` or `"city2026"` and returns only rows for that tag.
